...
class UserServiceImpl(
    ...
    @OptIn(ExperimentalUuidApi::class)
    override suspend fun deleteUser(id: String): DivaResult<ApiResponse<Nothing>, DivaError.NetworkError> {
        return tryResult(
            onError = { e ->
                e.toDivaError().asNetworkError(HttpRequestMethod.DELETE, "/api/user")
            }
        ) {
            val parsedId = Uuid.parse(id)
            storage
                .delete(parsedId)
                .mapError { err ->
